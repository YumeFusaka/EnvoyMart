...
| **下游拖垮上游** | Sentinel `DegradeRule`（慢调用比例 + 异常比例，配在 `product.deductStock` 资源上）；扣库存被熔断时**快速失败**，不降级成"成功"。**限流在网关、熔断在服务侧**——熔断要针对具体的下游依赖，网关那层没有对应粒度 |
| **跨服务写坏数据** | Seata AT 的**全局锁**：RM 拦截本地事务 `commit()`，先向 TC 申请该行全局锁、拿到才提交，持有到全局事务结束。它守住的是「Redisson 释放 → 全局事务提交」那段毫秒级的缝——实测从未触发争用（Redisson 已把整个 checkout 串行化），但它确实在生效。**未测的风险**：全局锁冲突重试耗尽会抛 `LockConflictException` 并回滚该请求，只在 TC 变慢时出现 |
| **崩溃时补偿丢失** | 这是**已知缺口**：手写 Saga 的"待补偿"记账在内存 `ArrayList` 里，进程被 kill 就没了。当前靠 Seata 的 TC 持久化兜底；`SEATA_ENABLED=false` 时无任何保障。**正解是本地消息表**（记录与订单同一本地事务），比 Seata 便宜且覆盖同一窗口——**未实现** |
...
429 响应体由 `GatewayErrorHandler` 统一写出（`{"code":429,"msg":"请求过于频繁，请稍后再试"}`）。

> **曾经有个「无 token 被限流返回 200 空响应」的缺陷，已修**。
> 网关里原本有两处各自写响应：`JwtGatewayFilter` 直接 `setStatusCode(401); setComplete()`、
> Sentinel 走它自带的 block handler。而 Gateway 的过滤器链是**同步递归调用**的
> （`chain.filter()` 会立即执行后续过滤器的方法体），Sentinel 顺序最前、它的实现也是
> 「先调 `chain.filter()` 再做限流判定」——于是鉴权分支在限流判定**之前**就把响应提交了，
> 限流随后想写的 429 再也写不进去，Netty 以默认的 200 收尾。
> 实测规律：公开路由被限流 → 429 正常；带 token 被限流 → 429 正常；**无 token 被限流 → 200 空响应**。
> **修法**：拒绝一律改成抛异常，由 `GatewayErrorHandler`（`@Order(HIGHEST_PRECEDENCE)`）
> 作为唯一出口写响应；它里面还留了一道 `response.isCommitted()` 的兜底检查——
> 真有谁又提前提交了响应，会在日志里留下痕迹，而不是静默变成一个 200。
...
把埋点换算成钱。方法：清空指标 → 连打 16 轮真实对话（两组各 8 轮，覆盖日常问答与工具密集链路，**同一 sessionId 让历史累积**）→ 读 `agent_llm_tokens` 差量。

| | 日常问答 | 工具密集 | 平均 |
|---|---|---|---|
| prompt / 轮 | 2,403 | 2,726 | **2,565** |
| completion / 轮 | 295 | 278 | **286** |
| 模型调用 / 轮 | 3.5 | 3.9 | **3.7** |

按 qwen-plus 华北2 的阶梯价（0–128K：输入 0.8 元 / 百万，输出 2 元 / 百万）：

    输入  2,565 × 0.0000008 = ¥0.002052
    输出    286 × 0.0000020 = ¥0.000572
    ─────────────────────────────────────
    单轮合计                ≈ ¥0.0026（约 0.26 分）
    千轮                    ≈ ¥2.6
    万轮                    ≈ ¥26

> **两个观察**：
> ① **成本的大头在输入**（0.00205 / 0.00262 ≈ **78%**）。所以真要降本，先看前缀缓存（system prompt + 工具定义每次都重发），而不是压输出长度。
> ② **"单次对话 5~10 次模型调用"这个说法测不出来**——实测平均 **3.7 次**（日常 3.5、工具密集 3.9）。旧数字是估的，已按实测改。
>
> **这份模型的边界**：语料只有 90 篇、工具集也只有 4 个，真实生产里 system prompt 与工具定义都会更长；另外它没算向量化与重排（每轮各一次，但查询短，量级比对话小两个数量级）。
...
2. **可复现的分层检索评测** —— `RetrievalEvaluator` 在 **90 篇语料、120 条标注样本**上算 Hit Rate@K / MRR / NDCG@K，样本按「字面 / 改写 / 语义」三档分层（各 40 条），避免一个平均分掩盖真实分布。语料**按主题成簇**（售后、物流、营销等九类各 6~14 篇），使同一条查询的候选里出现多篇语义相邻的文档——真实知识库就是这样，而不是一问一答。实测（纯关键词路，随机基线 **0.036**）：
   ...
   **端到端回答质量（`RagAnswerQualityTest`）**——上面那些是**中间指标**：召回到了正确文档，模型仍可能答错。所以又补了一层，直接评"用户拿到的回答"：
   ...
   做法：21 条样本（三档各 7 条，固定步长取样保证可复现）→ 检索 → 按线上 `buildSystemPrompt` 的「## 相关知识」段拼 system prompt → 生成回答 → 交给裁判模型，连同**夹具标注的标准文档正文**一起评忠实度与相关性。**只要问题、不要参考答案**，避开"参考答案本身成为偏差来源"。

   > **这张表最该看的是"检索指标涨了、回答质量几乎没动"**：Hit Rate 从 0.633 到 0.850（**+22pp**），换来的相关性只有 4.05 → 4.19，**忠实度甚至持平**。重排让 Hit Rate 再 +2.5pp，忠实度反而从 3.10 掉到 2.95。
   >
   > **更值得注意的是四个配置的忠实度都在 3/5 上下**——说明"模型经常说文档里没有的话"是**生成侧的系统性问题**，检索配置怎么调都盖不住。这直接修正了一个旧判断：先前材料里写"当前瓶颈在召回不在生成"，按这份数据看，**瓶颈至少在两边**。
   >
   > **@5 那一行再次印证前面的结论**：忠实度 2.95 → 2.81、相关性 4.19 → 4.05、token +18%，而回答变长了（274 → 335 字符）却更不忠实——**更多上下文带来的是更多可编造的素材**。
   >
   > **边界（必须一起说）**：样本只有 21 条，0.15 分的差距**在噪声范围内，不能声称显著**；裁判与生成是同一模型家族，有自我偏好；它只跑 RAG 那条路径，**不含意图路由、工具与多轮记忆**，所以它是"检索配置对回答质量的影响"，不是"整个 Agent 的回答质量"。
...
| **Seata 的评估结论建立在错误前提上** | 曾据"装上 Seata 后成功数从 10 掉到 3~7"判断它与 Redisson 锁双重加锁、吞吐腰斩，并回退了它 | 那个 3~7 其实是上面那条测试缺陷在不同轮次的表现。修掉测试后重测，**Seata 开关与否都是成功 10 / 库存归零**；它的真实代价只在耗时上，且波动（1.0~4.2s）大于配置差异，不足以支撑"慢多少"。**Seata 提供的是手写 Saga 没有的那一段：崩溃可恢复**——进程被 kill 后 TC 仍能驱动回滚。**但要说清：本地消息表能用一张表 + 一个定时任务拿到同一件事，而且更便宜**；Seata 多出来的"跨服务写隔离"在本项目增量极小（扣减已是原子 SQL）。所以留着它是"为演示这套能力"，不是"工程上的最优解" |
