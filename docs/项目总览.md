...
**加追踪的代价（实测）**：javaagent 给每个方法插桩，压测数字会失真——30 并发抢 10 件库存的成功数从稳定的 10 掉到 3。**观测本身有成本，性能验证时要关掉**（`ENVOYMART_SKYWALKING=off`），否则会把观测开销误读成业务缺陷。这一点在排查时绕过弯路：先怀疑 Seata、缓存互斥、事件发布、Redisson 连接池，逐个证伪后才定位到 agent。
...
2. **可复现的分层检索评测** —— `RetrievalEvaluator` 在 **90 篇语料、120 条标注样本**上算 Hit Rate@K / MRR / NDCG@K，样本按「字面 / 改写 / 语义」三档分层（各 40 条），避免一个平均分掩盖真实分布。语料**按主题成簇**（售后、物流、营销等九类各 6~14 篇），使同一条查询的候选里出现多篇语义相邻的文档——真实知识库就是这样，而不是一问一答。实测（纯关键词路，随机基线 **0.036**）：
   ...
   > **为什么顺带也打一份 @5**：外部材料常报 Hit@5，而 @3 与 @5 不是同一把尺子，没有同一语料上的 @5 就没法直接比。实测关键词路 **@3 0.633 → @5 0.675（+4.2pp）**，随机基线 0.036 → 0.059。**@3 到 @5 的增幅很小**，说明短板不在"正确文档排在第 4、5 位"，而在语义档压根没召回——换更宽的口径抬不上去，指标不是靠窄口径撑的。
...
| **接了链路追踪，压测数字直接腰斩** | 30 并发抢 10 件，成功数从稳定的 10 掉到 3，看起来像业务缺陷 | SkyWalking javaagent 给每个方法插桩，请求耗时被拉长，Redisson 锁等待（3 秒）跟着超时，真正走到扣库存那一步的只剩 9 个。**逐个证伪**才定位到它：先试 Seata、缓存互斥、事件发布、Redisson 连接池，排除完才轮到 agent。修法：压测时 `ENVOYMART_SKYWALKING=off`。**观测有成本，性能验证必须把观测关掉** |
| **Seata AT 装上后吞吐反而降**（评估后回退） | 30 并发抢 10 件库存，成功数 10 → 3~7（三次运行 7/6/3） | 它与 Redisson 锁在**串行化同一件事**：下单先拿 Redisson 锁（tryLock 等 3 秒），Seata 的 AT 又对同一行加全局锁，双重开销让锁持有时间变长。没有超卖，但吞吐腰斩。而项目已有的手写 Saga（显式记账 + 反序补偿）实测是好的。**加中间件不是免费的**——能复现这次评估是保留 Seata 配置的唯一理由（见 `docker-compose.yml` 注释） |
