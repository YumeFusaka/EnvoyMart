...
**语义鸿沟组的 0.100 低于随机基线**（30 篇取 top-3、按各样本的标注相关文档数逐例计算的随机命中率 = 0.116）——在语义化提问面前，纯关键词检索不只是退化成随机，它还会**自信地返回错的那篇**。

> 该测试的向量库是空的，**隔离测量的是关键词路的底线**。

**引入向量与重排后的对照**（`RetrievalComparisonTest`，本地跑）：

> 本夹具 30 篇文档最长仅 47 字符，**每篇恰好一个切片**。这解释了两件事：一是 `chunkId` 与 `docId` 取什么都
...
向量混合召回带来 **+23.4pp** 全量提升（语义档 +50pp）——**这两组数字四次重复运行完全一致**。

**重排档不可复现**：同一份代码同一套数据跑四次，语义档在 0.600 与 0.700 之间跳、全量在 0.867 与 0.900 之间跳，同一次运行内部还自相矛盾（分档相加 26/30，全量报 0.900）。给重排器加上生效/降级计数后确认**降级 0 次**，排除了"静默退化成不重排"这个解释。**结论：这套实验量不出重排的稳定效果，两个方向都不成立。** 完整分析见 [docs/检索效果对照实验结果.md](docs/检索效果对照实验结果.md)。
