...
**切分质量**（`ChunkingQualityTest` / `ChunkingRetrievalComparisonTest`）——上面那套测的是"检索算法"，
**测不到"切分策略"**：夹具里每篇恰好一片，切分逻辑从未被触发。度量切分需要**长文档**语料：

| 指标 | 定长 256/32（原实现） | 结构分层 512/40（线上） |
|---|---|---|
| 句末标点（切片层） | 30% | **100%** |
| 语义完整性（18 条：同条款关键信息是否同片） | 17/18 | **18/18** |
| 端到端完整召回（120 条查询） | 73.3% | **85.0%** |
...
> 切片级入口（`HybridRetriever.overChunks`，按 chunkId 归一）。
